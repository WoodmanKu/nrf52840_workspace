...
2. use ppi channel to simulate power down with button press
3. no more selfish talk   fixed, more stable now
...
static const nrf_drv_timer_t CD_60s = NRF_DRV_TIMER_INSTANCE(2); //60sec count-down, use 5 sec for testing
...
/**
 * @brief Handler for 60s power down
 */
void timer_60s_event_handler(nrf_timer_event_t event_type, void *p_context)
{
    // stop the esb entry loop and turn off all the leds
    entry_flagger = false;
    entry_flag = false;
    nrf_drv_timer_disable(&TIMER_ESB_ENTRY);
    // bsp_board_leds_on();  //for testing
}
...
static void timer_esb_entry_init(void){
    nrf_drv_timer_config_t timer_cfg = NRF_DRV_TIMER_DEFAULT_CONFIG;
        timer_cfg.frequency = NRF_TIMER_FREQ_1MHz;      ///< Timer frequency 1 MHz.
...
/**
 * @brief Configure TIMER with ppi channel for "power down" after 60s in entry loop
 */
static void entry_time_out(void) {
    // //set up ppi channel
    // nrf_ppi_channel_t ppi_channel_tout;
    ...
    // err_code = nrf_drv_ppi_channel_alloc(&ppi_channel_tout);
    // APP_ERROR_CHECK(err_code);

    //timer for 15s countdown
    nrf_drv_timer_config_t timer_cfg = NRF_DRV_TIMER_DEFAULT_CONFIG; //using default frequency 16MHz
        timer_cfg.frequency = NRF_TIMER_FREQ_31250Hz;    ///< Timer frequency 31250 Hz.
    err_code = nrf_drv_timer_init(&CD_60s, &timer_cfg, timer_60s_event_handler);
    APP_ERROR_CHECK(err_code);

    uint32_t time_ms = 5000; //set 15s for testing
    uint32_t time_ticks = nrf_drv_timer_ms_to_ticks(&CD_60s, time_ms);
    nrf_drv_timer_extended_compare(
        &CD_60s, NRF_TIMER_CC_CHANNEL2, time_ticks, NRF_TIMER_SHORT_COMPARE0_CLEAR_MASK, true);

    // err_code = nrf_drv_ppi_channel_assign(ppi_channel_tout, nrf_drv_timer_event_address_get(&CD_60s, NRF_TIMER_EVENT_COMPARE0));
    // APP_ERROR_CHECK(err_code);

    // err_code = nrf_drv_ppi_channel_enable(ppi_channel_tout);
    // APP_ERROR_CHECK(err_code);
}
        ...
        while (entry_flagger == true)
        {
            ...
            if ((count_try % 5) == 0)
            {
                ...
                } else if (count_success > count_fail) //valid timeslot , let's go!!
                {
                    ...
                    //set timer to fit sending loop
                    nrf_drv_timer_disable(&CD_60s);
                    nrf_drv_timer_disable(&TIMER_ESB_ENTRY);
